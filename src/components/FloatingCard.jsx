...
export default function FloatingCard({
  ...
  const [title, setTitle] = useState(`Video ${index + 1}`)
  const [isDragging, setIsDragging] = useState(false)
  const [hasLoaded, setHasLoaded] = useState(false)
  const didDragRef = useRef(false)

  // Per-card variation so they bob at different speeds / heights
  const bobDuration = 2.2 + index * 0.38
  const bobAmount  = 7  + (index % 3) * 3

  // Fetch real title from YouTube oEmbed
  useEffect(() => {
    const url =
  ...
  return (
    <>
      <motion.div
        // Staggered fade + scale in on page load
        initial={{ opacity: 0, scale: 0.45 }}
        animate={{
          opacity: isSelected ? 0 : 1,
          scale:   isSelected ? 0.88 : 1,
          // Disable pointer events while any card is expanded
          pointerEvents: isSelected || isAnySelected ? 'none' : 'auto',
        }}
        transition={{
          opacity: {
            delay:    hasLoaded ? 0 : index * 0.13,
            duration: 0.35,
          },
          scale: {
            delay:     hasLoaded || isSelected ? 0 : index * 0.13,
            type:      'spring',
            stiffness: 290,
            damping:   22,
          },
        ...
        onDrag={(_, info) => {
          if (
            Math.abs(info.offset.x) > 4 ||
            Math.abs(info.offset.y) > 4
          ) {
            didDragRef.current = true
          }
        }}
        onDragEnd={() => {
          // Let the click event fire first, then reset flag
      ...
      >
        ...
        <motion.div
          animate={{
            y: isDragging || isSelected ? 0 : [0, -bobAmount, 0],
          }}
          transition={
      ...
      <AnimatePresence>
        {isSelected && (
          ...
          >
            {/* Inner: handles spring scale/translate animation */}
            <motion.div
            ...
            >
              ...
              {/* YouTube iframe — autoplay=1 kicks in after user gesture (the click) */}
              <iframe
