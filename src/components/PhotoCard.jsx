...
export default function PhotoCard({
  ...
  const [initPos] = useState(() => {
    try { return JSON.parse(localStorage.getItem(`wl-dp-${photo.id}`)) ?? { x: 0, y: 0 } }
        ...
        {isSelected && (
          ...
          >
            <motion.div
              ...
              style={{ pointerEvents: 'all', width: 'min(800px, 92vw)', borderRadius: 18, overflow: 'hidden', background: '#111', boxShadow: '0 40px 100px rgba(0,0,0,0.62), 0 10px 32px rgba(0,0,0,0.38)', position: 'relative' }}
            >
              <button
                onClick={onClose}
              ...
              >×</button>
              <img src={photo.src} alt={photo.alt} style={{ width: '100%', display: 'block', maxHeight: '85vh', objectFit: 'contain' }} />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
